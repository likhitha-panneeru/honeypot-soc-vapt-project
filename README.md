...
Repository Contents
File	Description
Final_Project_Report.docx	Consolidated findings report — honeypot, cloud data, VAPT results, recommendations
Honeypot_Project_Report.docx	Detailed academic-style report on the honeypot component
cloud_vapt_soc_guide.html	Implementation guide for the AWS-native VAPT + SOC architecture
analyze_cowrie.py	Python script that parses Cowrie's JSON logs and generates threat intelligence statistics and charts
cowrie.json	Local honeypot log data (simulated attack sessions)
cowrie_cloud_full.json	Cloud honeypot log data (real, unsolicited internet attacker activity)
cowrie_teammate_bruteforce.json	Log data from a controlled team-member brute-force test
vampi_logs_for_splunk.txt	VAmPI attack logs, including automated sqlmap exploitation traffic
chart_*.png / chart_*_CLOUD.png	Generated visualizations: event types, top usernames/passwords, attack timeline (local and cloud datasets)
Methodology
